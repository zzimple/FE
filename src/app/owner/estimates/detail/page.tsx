...
export default function OwnerEstimateDetailPage() {
  ...
  // API 데이터 가져오기
  useEffect(() => {
    if (!estimateNo || isNaN(estimateNo) || estimateNo <= 0 || !storeId) {
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 pt-12">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 space-y-6">
            ...
            {/* 지도 */}
            <div className="space-y-1">
              <div className="text-sm font-semibold text-gray-900">이동 경로</div>
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                <KakaoMapRoute 
                  estimateNo={Number(reviewData.estimateNo)}
              ...
              {(duration || distance) && (
                <div className="flex gap-4 text-base font-bold text-gray-700">
                  {duration && <span>예상 소요시간: {Math.round(duration / 60)}분</span>}
                  {distance && <span>예상 거리: {Math.round(distance / 1000)}km</span>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mt-8 flex gap-4">
            {/* 견적서 목록 버튼 */}
            <Link href="/owner/estimates" className="flex-1">
