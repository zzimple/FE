...
export default function BasicPricePage() {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      try {
        const response = await authApi.get("/owner/my/basic-price");
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await authApi.post("/owner/my/basic-price", updatedPrices);
      if (response.data.success) {
        alert("기본금 설정이 저장되었습니다.");
        setEditMode(false);
        setChanges({});
        const newResponse = await authApi.get("/owner/my/basic-price");
